...
public class ChatbotEngine {
    ...
    private void handleConversationFlow(String userMessage,
        ...
        switch (state) {
            ...
            case WAITING_PATIENT_CODE:
                handlePatientCodeInput(userMessage, callback);
                break;
    ...
    private void handleNewIntent(IntentDetector.Intent intent,
        ...
        switch (intent) {
            ...
            case XEM_BENH_AN:
                if ("benhnhan".equals(currentUserType) || "bacsi".equals(currentUserType)) {
    ...
    private void handleViewMedicalRecord(ChatCallback callback) {
        ...
        // Nếu là bác sĩ, cần nhập mã bệnh nhân
        if ("bacsi".equals(currentUserType)) {
            ...
            // Chuyển sang trạng thái chờ nhập mã bệnh nhân
            conversationContext.setState(ConversationContext.ConversationState.WAITING_PATIENT_CODE);
            conversationContext.setData("pendingAction", "XEM_BENH_AN");
            
            ChatResponse response = new ChatResponse(
                "👨‍⚕️ XEM BỆNH ÁN BỆNH NHÂN\n\n" +
                "Vui lòng nhập mã bệnh nhân để xem bệnh án:",
                ChatResponse.ResponseType.TEXT
            );
            callback.onResponse(response);
            return;
        ...
        // Nếu là bệnh nhân, xem bệnh án của chính mình
        if (maBenhNhan == null || maBenhNhan.isEmpty()) {
            ChatResponse response = new ChatResponse(
                "❌ Không tìm thấy thông tin bệnh nhân. Vui lòng đăng nhập lại.",
                ChatResponse.ResponseType.TEXT
        ...
        loadMedicalRecordForPatient(maBenhNhan, callback);
    }
            querySnapshot -> {
    ...
    private void handleViewPrescriptions(ChatCallback callback) {
        ...
        if ("bacsi".equals(currentUserType)) {
            ...
            // Chuyển sang trạng thái chờ nhập mã bệnh nhân
            conversationContext.setState(ConversationContext.ConversationState.WAITING_PATIENT_CODE);
            conversationContext.setData("pendingAction", "XEM_DON_THUOC");
            
            ChatResponse response = new ChatResponse(
                "👨‍⚕️ XEM ĐỚN THUỐC BỆNH NHÂN\n\n" +
                "Vui lòng nhập mã bệnh nhân để xem đơn thuốc:",
                ChatResponse.ResponseType.TEXT
            );
            callback.onResponse(response);
            return;
        ...
        // Nếu là bệnh nhân, xem đơn thuốc của chính mình
        if (maBenhNhan == null || maBenhNhan.isEmpty()) {
            ChatResponse response = new ChatResponse(
                "❌ Không tìm thấy thông tin bệnh nhân. Vui lòng đăng nhập lại.",
                ChatResponse.ResponseType.TEXT
        ...
        loadPrescriptionsForPatient(maBenhNhan, callback);
    }
            querySnapshot -> {
    ...
    // ============================================
    // ENHANCED METHODS FOR DOCTOR-PATIENT ACCESS
    // ============================================
    
    /**
     * Xử lý nhập mã bệnh nhân cho bác sĩ
     */
    private void handlePatientCodeInput(String userMessage, ChatCallback callback) {
        String patientCode = userMessage.trim();
        
        if (patientCode.length() < 3) {
            ChatResponse response = new ChatResponse(
                "❌ Mã bệnh nhân không hợp lệ\n\n" +
                "Vui lòng nhập mã bệnh nhân hợp lệ (ít nhất 3 ký tự):",
                ChatResponse.ResponseType.TEXT
            );
            callback.onResponse(response);
            return;
        ...
        repo.getByField("BenhNhan", "maBenhNhan", patientCode,
            querySnapshot -> {
                if (querySnapshot.isEmpty()) {
                    ChatResponse response = new ChatResponse(
                        "❌ Không tìm thấy bệnh nhân\n\n" +
                        "Mã bệnh nhân \"" + patientCode + "\" không tồn tại trong hệ thống.\n\n" +
                        "Vui lòng kiểm tra lại hoặc nhập mã khác:",
                        ChatResponse.ResponseType.QUICK_REPLY
                    );
                    
                    List<String> quickReplies = new ArrayList<>();
                    quickReplies.add("🔄 Nhập lại mã");
                    quickReplies.add("❌ Hủy bỏ");
                    response.setQuickReplies(quickReplies);
                ...
                com.example.doannt118.model.BenhNhan benhNhan = doc.toObject(com.example.doannt118.model.BenhNhan.class);
                
                if (benhNhan != null) {
                    // Lưu mã bệnh nhân tạm thời
                    conversationContext.setData("tempPatientCode", patientCode);
                    conversationContext.setData("tempPatientName", benhNhan.getHoTen());
                    
                    // Thực hiện hành động đã lưu
                    String pendingAction = (String) conversationContext.getData("pendingAction");
                    conversationContext.setState(ConversationContext.ConversationState.IDLE);
                    
                    ChatResponse confirmResponse = new ChatResponse(
                        "✅ Tìm thấy bệnh nhân\n\n" +
                        "👤 Tên: " + benhNhan.getHoTen() + "\n" +
                        "🆔 Mã BN: " + patientCode + "\n\n" +
                        "Đang tải thông tin...",
                        ChatResponse.ResponseType.TEXT
                    );
                    callback.onResponse(confirmResponse);
                    
                    // Thực hiện hành động tương ứng
                    if ("XEM_BENH_AN".equals(pendingAction)) {
                        loadMedicalRecordForPatient(patientCode, callback);
                    } else if ("XEM_DON_THUOC".equals(pendingAction)) {
                        loadPrescriptionsForPatient(patientCode, callback);
                    } else if ("XEM_HOA_DON".equals(pendingAction)) {
                        loadInvoicesForPatient(patientCode, callback);
                    }
                }
                
            }, e -> {
                Log.e("ChatbotEngine", "Error checking patient code: ", e);
                
                ChatResponse response = new ChatResponse(
                    "❌ Lỗi kiểm tra mã bệnh nhân\n\n" +
                    "Không thể kiểm tra mã bệnh nhân. Vui lòng thử lại:",
                    ChatResponse.ResponseType.QUICK_REPLY
                );
    ...
    /**
     * Load bệnh án cho bệnh nhân cụ thể
     */
    private void loadMedicalRecordForPatient(String patientCode, ChatCallback callback) {
        repo.getByField("BenhAn", "maBenhNhan", patientCode,
            querySnapshot -> {
                List<com.example.doannt118.model.BenhAn> benhAnList = new ArrayList<>();
                ...
                if (benhAnList.isEmpty()) {
                    ...
                    List<String> quickReplies = new ArrayList<>();
                    quickReplies.add("📝 Tạo bệnh án mới");
                    quickReplies.add("💊 Xem đơn thuốc");
                    quickReplies.add("💰 Xem hóa đơn");
                    quickReplies.add("🔍 Tra cứu BN khác");
                ...
                responseText.append("🆔 Mã BN: ").append(patientCode).append("\n");
                
                String patientName = (String) conversationContext.getData("tempPatientName");
                if (patientName != null) {
                    responseText.append("👤 Tên: ").append(patientName).append("\n");
                }
    ...
    /**
     * Load đơn thuốc cho bệnh nhân cụ thể
     */
    private void loadPrescriptionsForPatient(String patientCode, ChatCallback callback) {
        repo.getByField("DonThuoc", "maBenhNhan", patientCode,
            querySnapshot -> {
                List<com.example.doannt118.model.DonThuoc> donThuocList = new ArrayList<>();
                ...
                if (donThuocList.isEmpty()) {
                    ...
                    List<String> quickReplies = new ArrayList<>();
                    quickReplies.add("💊 Kê đơn thuốc mới");
                    quickReplies.add("📋 Xem bệnh án");
                    quickReplies.add("💰 Xem hóa đơn");
                    quickReplies.add("🔍 Tra cứu BN khác");
                ...
                StringBuilder responseText = new StringBuilder();
                responseText.append("💊 ĐỚN THUỐC BỆNH NHÂN\n\n");
                responseText.append("🆔 Mã BN: ").append(patientCode).append("\n");
    ...
    /**
     * Load hóa đơn cho bệnh nhân cụ thể
     */
    private void loadInvoicesForPatient(String patientCode, ChatCallback callback) {
        repo.getByField("HoaDon", "maBenhNhan", patientCode,
            querySnapshot -> {
                List<com.example.doannt118.model.HoaDon> hoaDonList = new ArrayList<>();
                ...
                if (hoaDonList.isEmpty()) {
                    ChatResponse response = new ChatResponse(
                        "💰 HÓA ĐƠN BỆNH NHÂN\n\n" +
                        "🆔 Mã BN: " + patientCode + "\n" +
                        "📋 Bệnh nhân chưa có hóa đơn nào.\n\n" +
                        "Bạn có thể:",
                        ChatResponse.ResponseType.QUICK_REPLY
                    );

                    List<String> quickReplies = new ArrayList<>();
                    quickReplies.add("💰 Tạo hóa đơn mới");
                    quickReplies.add("📋 Xem bệnh án");
                    quickReplies.add("💊 Xem đơn thuốc");
                    quickReplies.add("🔍 Tra cứu BN khác");
                ...
                StringBuilder responseText = new StringBuilder();
                responseText.append("💰 HÓA ĐƠN BỆNH NHÂN\n\n");
                responseText.append("🆔 Mã BN: ").append(patientCode).append("\n");
    ...
    private void handleViewInvoices(ChatCallback callback) {
        ...
        if ("bacsi".equals(currentUserType)) {
            ...
            // Chuyển sang trạng thái chờ nhập mã bệnh nhân
            conversationContext.setState(ConversationContext.ConversationState.WAITING_PATIENT_CODE);
            conversationContext.setData("pendingAction", "XEM_HOA_DON");
            
            ChatResponse response = new ChatResponse(
                "👨‍⚕️ XEM HÓA ĐƠN BỆNH NHÂN\n\n" +
                "Vui lòng nhập mã bệnh nhân để xem hóa đơn:",
                ChatResponse.ResponseType.TEXT
            );
            callback.onResponse(response);
            return;
        ...
        // Nếu là bệnh nhân, xem hóa đơn của chính mình
        if (maBenhNhan == null || maBenhNhan.isEmpty()) {
            ChatResponse response = new ChatResponse(
                "❌ Không tìm thấy thông tin bệnh nhân. Vui lòng đăng nhập lại.",
                ChatResponse.ResponseType.TEXT
        ...
        loadInvoicesForPatient(maBenhNhan, callback);
    }
}