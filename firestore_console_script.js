...
// Vào Firebase Console → Firestore Database → Data tab
...
// Thêm từng document một cách tuần tự
async function addDuocPhamData() {
  const db = firebase.firestore();
  
  for (let i = 0; i < duocPhamData.length; i++) {
    const duocPham = duocPhamData[i];
    try {
      ...
      console.error(`✗ Lỗi khi thêm ${duocPham.maDuocPham}:`, error);
    }
  }
  
  console.log(`\n🎉 Hoàn thành! Đã thêm ${duocPhamData.length} dược phẩm vào collection DuocPham`);
}
